...
-(void)sendP2PFileWithId:(NSNumber * _Nonnull)userId
    ...
    if (isAudio == NO && isFile == NO) {
        ...
        RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm sendP2PFileWithId error. invalid parameter  (pid:%lld)",self.projectId]))
        return;
    }
    ...
    [self getP2PFileTokenWithId:userId
        ...
        if (RTMNullString(data[@"endpoint"]) || RTMNullString(data[@"token"])) {
            ...
            RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm sendFile error. getP2PFileTokenWithId return data is nil  (pid:%lld)",self.projectId]))
            return ;
        }
        ...
        if (isAudio == YES) {//audio
            ...
            if (audioData == nil) {
                ...
                RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm P2P audioMessage get audioData error  (pid:%lld)",self.projectId]))
                return ;
            }
            if ([RTMAudioTools isAmrVerify:audioData] == NO) {
                FPNSLog(@"rtm P2P sendAudioMessageChatWithId no amr type");
                RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm P2P sendAudioMessageChatWithId no amr type  (pid:%lld)",self.projectId]))
                return ;
            }
...
//
        ...
        FPNNQuest * quest = [FPNNQuest questWithMethod:@"sendfile" message:resultBody twoWay:YES pid:[NSString stringWithFormat:@"%lld",self.projectId]];
...
//    NSLog(@"%@  %@  %@  %@",groupId,fileData,fileName,fileSuffix);
    ...
    if (isAudio == NO && isFile == NO) {
        ...
        RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm sendGroupFileWithId error. invalid parameter  (pid:%lld)",self.projectId]))
        return;
    }
    ...
    [self getGroupFileTokenWithId:groupId
        ...
        if (RTMNullString(data[@"endpoint"]) || RTMNullString(data[@"token"])) {
            ...
            RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm sendFile error. getGroupFileToken return data is nil  (pid:%lld)",self.projectId]))
            return ;
        }
        ...
        if (isAudio == YES) {//audio
            ...
            if (audioData == nil) {
                ...
                RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm group audioMessage get audioData error (pid:%lld)",self.projectId]))
                return ;
            }
            if ([RTMAudioTools isAmrVerify:audioData] == NO) {
                FPNSLog(@"rtm group sendAudioMessageChatWithId no amr type");
                RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm group sendAudioMessageChatWithId no amr type  (pid:%lld)",self.projectId]))
                return ;
            }
...
-(void)sendRoomFileWithId:(NSNumber * _Nonnull)roomId
    ...
    if (isAudio == NO && isFile == NO) {
        ...
        RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm sendRoomFileWithId error. invalid parameter  (pid:%lld)",self.projectId]))
        return;
    }
    ...
    [self getRoomFileTokenWithId:roomId
        ...
        if (RTMNullString(data[@"endpoint"]) || RTMNullString(data[@"token"])) {
            ...
            RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm sendFile error. getRoomFileTokenWithId return data is nil (pid:%lld)",self.projectId]))
            return ;
        }
        ...
        if (isAudio == YES) {//audio
            ...
            if (audioData == nil) {
                ...
                RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm room audioMessage get audioData error  (pid:%lld)",self.projectId]))
                return ;
            }
            if ([RTMAudioTools isAmrVerify:audioData] == NO) {
                FPNSLog(@"rtm room sendAudioMessageChatWithId no amr type");
                RtmFpnnErrorLog(([NSString stringWithFormat:@"rtm room sendAudioMessageChatWithId no amr type  (pid:%lld)",self.projectId]))
                return ;
            }
